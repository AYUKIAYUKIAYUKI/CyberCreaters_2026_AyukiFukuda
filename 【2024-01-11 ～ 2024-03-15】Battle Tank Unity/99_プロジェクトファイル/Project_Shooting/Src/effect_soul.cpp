...
/*---【マクロ定義】---*/
#define MAX_EFFECT_SOUL 256	// 魂エフェクトの最大数

/*---【グローバル変数】---*/
LPDIRECT3DVERTEXBUFFER9 g_pVtxBuffEffect_Soul = NULL;	// 頂点バッファへのポインタ
LPDIRECT3DTEXTURE9 g_pTextureEffect_Soul = NULL;		// テクスチャへのポインタ
Effect_Soul g_Effect_Soul[MAX_EFFECT_SOUL];				// 魂エフェクト情報

/*---【プロトタイプ宣言】---*/

// 初期
void ImportTexEffect_Soul(void);		// テクスチャ取得
void CreateVtxBufferEffect_Soul(void);	// 頂点情報を生成

// 更新
void SetVtxEffect_Soul(void);	// 展開

void AnimationManager_Soul(int id);	// アニメーション動作管理用
void AnimationPlay_Soul(int id);	// 魂：アニメーション動作用

TextureSize SetTexEffect_Soul(void);	// テクスチャ管理用

void DeleteEffect_Soul(int id);	// 消去
...
void InitEffect_Soul(void)
{
	...
	{ // 魂エフェクト情報リセット
		...
		g_Effect_Soul[i].use = false;								// 使用フラグ
		g_Effect_Soul[i].pPos = nullptr;							// 対象位置
		g_Effect_Soul[i].pos_moved = { 0.0f, 0.0f, 0.0f };			// 変動位置
		g_Effect_Soul[i].pos = { 0.0f, 0.0f, 0.0f };				// 位置
		g_Effect_Soul[i].rot = { 0.0f, 0.0f, 0.0f };				// 向き
		g_Effect_Soul[i].span = { 0.0f, 0.0f };						// 大きさ用
		g_Effect_Soul[i].angle = 0.0f;								// 角度
		g_Effect_Soul[i].length = 0.0f;								// 対角線用
		g_Effect_Soul[i].col = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);	// 色
		g_Effect_Soul[i].remain = 0;								// 寿命

		g_Effect_Soul[i].anima.row = 0;			// 行
		g_Effect_Soul[i].anima.column = 0;		// 列
		g_Effect_Soul[i].anima.id_pattern = 0;	// 現在のパターン
		g_Effect_Soul[i].anima.dur_cnt = 0;		// パターン遷移間隔
		g_Effect_Soul[i].anima.id_cnt = 0;		// 遷移カウンター
	}
...
{
	g_Effect_Soul[id].use = false;								// 使用フラグ
	g_Effect_Soul[id].pPos = nullptr;							// 対象位置
	g_Effect_Soul[id].pos_moved = { 0.0f, 0.0f, 0.0f };			// 変動位置
	g_Effect_Soul[id].pos = { 0.0f, 0.0f, 0.0f };				// 位置
	g_Effect_Soul[id].rot = { 0.0f, 0.0f, 0.0f };				// 向き
	g_Effect_Soul[id].span = { 0.0f, 0.0f };					// 大きさ用
	g_Effect_Soul[id].angle = 0.0f;								// 角度
	g_Effect_Soul[id].length = 0.0f;							// 対角線用
	g_Effect_Soul[id].col = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);	// 色
	g_Effect_Soul[id].remain = 0;								// 寿命

	g_Effect_Soul[id].anima.row = 0;		// 総行数
	g_Effect_Soul[id].anima.column = 0;		// 総列数
	g_Effect_Soul[id].anima.id_pattern = 0;	// 現在のパターン
	g_Effect_Soul[id].anima.dur_cnt = 0;	// パターン遷移間隔
	g_Effect_Soul[id].anima.id_cnt = 0;		// 遷移カウンター
}
...
void DrawEffect_Soul(void)
{
	...
	for (int i = 0; i < MAX_EFFECT_SOUL; i++)
	{
		...
		// ポリゴンをカメラの正面に向け続ける
		D3DXMatrixInverse(&g_Effect_Soul[i].mtx, NULL, &mtxView);	// 逆行列を求める
...
int CreateEffect_Soul(D3DXVECTOR3* pPos)
{
	...
	for (int i = 0; i < MAX_EFFECT_SOUL; i++)
	{
		...
		g_Effect_Soul[i].pPos = pPos;	// 対象位置を取得

		g_Effect_Soul[i].span = { 10.0f, 25.0f };	// 大きさ：適量
		g_Effect_Soul[i].remain = 220;				// 寿命：適量
